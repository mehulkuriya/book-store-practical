...
<div class="card">
  ...
  <div class="form-outline mb-4">
        <input type="search" id="form1" name="search"  v-model="search"  class="form-control" v-on:change="searchBook" />
    </div>
        ...
        <table class="table table-hover table-sm">
            ...
            <tbody>
                <tr v-for="(book, index) in books" :key="book.id">
                    ...
                    <td> <router-link :to="{name:'detailbook', params: {id:book.id}}" target="_blank">{{book.name}}</router-link></td>
                    <td>{{book.author}}</td>
                    <td>{{book.genre}}</td>
                     <td>{{book.isbn}}</td>
